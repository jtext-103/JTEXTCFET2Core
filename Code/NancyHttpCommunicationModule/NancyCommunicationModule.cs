        ...
        private ISample HTTPRequest(string method, string requestUri, Dictionary<string, object> inputDict, params object[] inputs)
        {
            ...
            try
            {
                if (resp.ContentType.Contains("MessagePack"))
                {
                    ...
                    result.Context = MessagePackSerializer.Deserialize<Dictionary<string, object>>(realStream, MessagePack.Resolvers.ContractlessStandardResolver.Options);
                    var contextIgnoreCase = new Dictionary<string, object>(StringComparer.InvariantCultureIgnoreCase);
                    foreach (var pair in result.Context)
                    {
                        contextIgnoreCase.Add(pair.Key, pair.Value);
                    }
                    result.Context = contextIgnoreCase;
                }
                else
                {
